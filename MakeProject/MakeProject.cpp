...
fs::path configFileName = L"config.dat";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    static TCHAR szMutexName[256] = L"MakeProject";
    ...
    case WM_CREATE:
    {
        RegisterHotKey(hwnd, IDC_CTRL_F5_HOTKEY, MOD_CONTROL, VK_F2);
        ...
        if (bm.ButtonSize())
        {
            fs::copy(configParentPath / configFileName, configParentPath / L"backup.dat",
                fs::copy_options::update_existing);
        }

        else if ((bm.ButtonSize() == 0) && (fs::exists(configParentPath / L"backup.dat")))
        {
            fs::remove(configParentPath / configFileName);
            fs::rename(configParentPath / L"backup.dat", configParentPath / configFileName);
            fs::copy(configParentPath / configFileName, configParentPath / L"backup.dat", 
                fs::copy_options::update_existing);
        }
...
//void ActiveThread(HWND hwnd)
//{
//    static POINT beforePoint{ 0, 0 };
//    static POINT point{0, 0};
//    static int acvCount = 0;
